<script lang="ts">
  import type { ExamTable } from "../../types/report-examtable.types";
  ...
  <div class="flex justify-between">
    ...
    <div class="text-right my-auto">
      <h1 class="text-2xl font-bold">
        {type === 'M' ? 'สอบกลางภาค' : 'สอบปลายภาค'} {studentInfo.semester}/{studentInfo.year}
      </h1>

      <p class="text-sm">{studentInfo.studentId} {studentInfo.name}</p>
      <p class="text-sm">
        คณะ{studentInfo.faculty} ภาควิชา{studentInfo.department}
      </p>
      <p class="text-sm">
        สาขา{studentInfo.curriculum}
      </p>
    </div>
  </div>
</div>
